...
|----------|------|------|
| 0 | 環境構築・骨格 | **完了** |
| 1 | バックエンド基盤（DB・設定・エラー処理） | **完了** |
| 2 | 認証（FastAPI Users） | **完了** |
| 3 | セッション API | **完了** |
| 4 | 会話 API（SSE + OpenAI） | **完了** |
| 5 | 会話後フィードバック API | **完了** |
| 6 | 保存・復習 API | **完了** |
| 7 | フロントエンド基盤（ルーティング・認証） | **完了** |
| 8 | ダッシュボード・セッション作成画面 | **完了** |
| 9 | 会話画面（Web Speech API） | **完了** |
| 10 | フィードバック・復習画面 | **完了** |
| 11 | OpenAPI 型生成の運用 | **完了** |
| 12 | 本番デプロイ | 未着手 |
...
### フェーズ 1〜6 で完了しているもの（バックエンド）

- `config.py` / `database.py` / Alembic マイグレーション 3 本
- 共通エラーハンドラ、リクエスト ID、構造化ログ、CORS
- FastAPI Users（JWT）+ ログインレート制限 + パスワードポリシー
- セッション / 会話（SSE）/ フィードバック / 保存 API 一式
- 詳細は [REFACTOR_20260623.md](./REFACTOR_20260623.md) および [ONBOARDING_READ_ORDER.md](./ONBOARDING_READ_ORDER.md) を参照
...
[6 完了] 保存・復習 API ───────────────────────┘
    ↓
[7 完了] フロントエンド基盤（ルーティング・認証）
    ↓
[8 完了] ダッシュボード・セッション作成
    ↓
[9 完了] 会話画面
    ↓
[10 完了] フィードバック・復習画面
    ↓
[11 完了] OpenAPI 型生成の運用整備
    ↓
[12] 本番デプロイ
```
...
- [x] 未ログインで `/dashboard` にアクセスすると `/login` へリダイレクトされる
- [x] 登録・ログイン後に `/dashboard` へ遷移できる
- [x] ランディングページが表示される
...
- [x] ダッシュボードに過去セッション一覧が表示される
- [x] 状況を入力して新規セッションを作成し、会話画面へ遷移できる
...
- [x] テキストまたは音声で発話し、AI の返答がストリーミング表示される
- [x] 会話終了後、フィードバック画面へ遷移できる
- [x] ページ再読み込みで過去メッセージが復元される（`GET /api/sessions/{id}`）
...
- [x] 主要 API 呼び出しが生成型を使用している（`src/types/*.ts` が `api.generated.ts` を参照）
- [x] バックエンド変更後に型再生成でビルドが通る

#### 型再生成手順

```bash
# バックエンド起動中に frontend/ で実行
npm run generate:api
```

生成先: `frontend/src/types/api.generated.ts`（`.gitignore` 対象）。  
ドメイン型は `src/types/sessions.ts` 等で `components['schemas'][...]` をエイリアスとして再エクスポートする。
...
**今すぐ着手するフェーズ: 12（本番デプロイ）**

1. Neon 本番 DB + マイグレーション適用
2. Railway（バックエンド）+ Cloudflare Pages（フロント）デプロイ
3. 本番環境変数・CORS 設定
4. E2E 手動テスト（登録 → 会話 → フィードバック → 復習）
...
|------|------|
| 2025-06-21 | 初版作成 |
| 2026-06-25 | 進捗サマリーを実装状況に合わせて更新（フェーズ 1〜6 完了、フェーズ 7 着手） |
| 2026-06-25 | フェーズ 9〜11 完了（会話・フィードバック・復習画面、OpenAPI 型適用） |
